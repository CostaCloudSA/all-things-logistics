...
Write-Host "`n[1/5] Setting active project to '$PROJECT_ID'..." -ForegroundColor Yellow
gcloud config set project $PROJECT_ID

# 2. Enable Required Cloud APIs
Write-Host "`n[2/5] Enabling Google Cloud Services..." -ForegroundColor Yellow
gcloud services enable run.googleapis.com cloudbuild.googleapis.com artifactregistry.googleapis.com bigquery.googleapis.com iam.googleapis.com

# 3. Create Artifact Registry Repository (if not exists)
Write-Host "`n[3/5] Checking Artifact Registry repository '$REPO_NAME'..." -ForegroundColor Yellow
$repoExists = gcloud artifacts repositories list --location=$REGION --filter="name:$REPO_NAME" --format="value(name)"
if (-not $repoExists) {
    gcloud artifacts repositories create $REPO_NAME --repository-format=docker --location=$REGION --description="Campabadal Logistics Container Registry"
...
Write-Host "`n[4/5] Building & Deploying Backend API..." -ForegroundColor Yellow
$BACKEND_IMAGE = "$REGION-docker.pkg.dev/$PROJECT_ID/$REPO_NAME/backend:latest"
gcloud builds submit backend --tag $BACKEND_IMAGE

gcloud run deploy $BACKEND_SERVICE --image $BACKEND_IMAGE --region $REGION --allow-unauthenticated --set-env-vars "ENVIRONMENT=production,PROJECT_ID=$PROJECT_ID" --memory 2Gi --cpu 2 --timeout 300

$BACKEND_URL = gcloud run services describe $BACKEND_SERVICE --region $REGION --format="value(status.url)"
Write-Host "   Backend API live at: $BACKEND_URL" -ForegroundColor Green

# 5. Build & Deploy Flutter Web Client
Write-Host "`n[5/5] Building & Deploying Flutter Web Client..." -ForegroundColor Yellow
$FRONTEND_IMAGE = "$REGION-docker.pkg.dev/$PROJECT_ID/$REPO_NAME/frontend:latest"
gcloud builds submit client --tag $FRONTEND_IMAGE

gcloud run deploy $FRONTEND_SERVICE --image $FRONTEND_IMAGE --region $REGION --allow-unauthenticated --memory 512Mi --cpu 1
